...
stateDiagram-v2
    ...
    Running --> Ready: Keep Warm (5-45min)
...
- Sau khi xử lý xong request, container được giữ **warm trong 5-45 phút**
- Requests tiếp theo trong thời gian này sẽ reuse container (warm start)
- Nếu không có request mới, container bị **frozen** rồi **shutdown**
...
| **1792 MB** | **1 full vCPU** | **⭐ Sweet spot cho CPU-intensive tasks** |
| 3008 MB | 1.73 vCPU | Heavy processing, image manipulation |
| 10240 MB | 6 vCPU | ML inference, video processing |

**Key Points**:
- ⭐ **1,792 MB = 1 full vCPU** (điểm quan trọng cho exam)
- Double memory = Double CPU power
- Không thể config CPU trực tiếp, chỉ có thể config memory
- CPU-bound tasks nên dùng >= 1792 MB để có full vCPU
...
**⭐ Timeout Limit**: API Gateway có **hard limit 29 seconds**
- Lambda max timeout là 15 phút
- Nhưng khi dùng với API Gateway: **chỉ có 29 giây**
- Nếu cần > 29s: dùng async pattern (SNS → Lambda) hoặc Step Functions
...
graph LR
    ...
    subgraph "Sweet Spot Example"
        ...
        H[Test at 1792MB: 500ms × $0.0000000292 = $0.0146]
    end
...
graph TB
    ...
    subgraph "Streaming Limits"
        ...
        K[SQS: 10,000 messages FIFO]
        L[SQS: 10 messages Standard]
    end
...
flowchart TD
    ...
    B --> E[Payload: 6MB sync, 256KB async]
...
3. ✅ **Payload Limits**: 6MB synchronous, 256KB asynchronous
