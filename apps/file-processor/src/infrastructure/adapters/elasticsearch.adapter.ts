...
export class ElasticsearchAdapter implements ISearchPort, OnModuleInit {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    const requestTimeout = parseInt(
      this.configService.get<string>("ELASTICSEARCH_REQUEST_TIMEOUT", "30000"),
      10,
    );
    ...
    this.logger.log(
      `Initializing Elasticsearch client with node: ${node}, requestTimeout: ${requestTimeout}`,
    );
    ...
    this.logger.log(
      `Elasticsearch Adapter initialized with index: ${this.indexName}`,
    );
  }
  ...
  private async ensureIndexExists(): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to ensure index exists: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async bulkIndexChunks(chunks: FileChunk[]): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to bulk index chunks: ${error.message}`,
        error.stack,
      );
    ...
    try {
      ...
      const chunks = response.hits.hits.map((hit: any) => {
        ...
        return new FileChunk(
          ...
          source.chunkIndex,
          source.content,
          source.startByte ?? source.position?.startByte ?? 0,
          source.endByte ?? source.position?.endByte ?? 0,
          source.metadata,
          new Date(source.createdAt || source.timestamp),
  ...
  async deleteFileChunks(fileId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to delete file chunks: ${error.message}`,
        error.stack,
      );
