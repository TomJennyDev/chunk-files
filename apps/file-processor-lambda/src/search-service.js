...
class SearchService {
  constructor(config = {}) {
    this.esClient = new Client({
      ...
      auth: {
        username:
          config.username || process.env.ELASTICSEARCH_USERNAME || "admin",
        password:
          config.password || process.env.ELASTICSEARCH_PASSWORD || "admin",
      },
      tls: {
    ...
    this.index =
      config.index || process.env.ELASTICSEARCH_INDEX || "file-chunks";
  }
  ...
  async hybridSearch(query, options = {}) {
    ...
    console.log(
      `Hybrid search: text(${textWeight}) + semantic(${semanticWeight})`,
    );
