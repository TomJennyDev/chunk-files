// ESM-only modules — loaded lazily via dynamic import()
let _textsplitters = null;
let _pipeline = null;
let _marked = null;

async function getTextsplitters() {
  if (!_textsplitters) {
    _textsplitters = await import("@langchain/textsplitters");
  }
  return _textsplitters;
}
...
async function initEmbeddingModel() {
  if (!embeddingModel) {
    ...
    embeddingModel = await pipelineFn(
      "feature-extraction",
 ...
 */
async function parseMarkdownStructure(content) {
...
async function createMarkdownChunks(content, options = {}) {
  ...
  console.log(
    `Parsed structure: ${structure.headings.length} headings, ${structure.sections.length} sections`,
  );

  // Use LangChain's MarkdownTextSplitter for semantic chunking
  const { MarkdownTextSplitter } = await getTextsplitters();
  ...
  const enrichedChunks = await Promise.all(
    documents.map(async (doc, index) => {
      ...
      const relevantHeading = findRelevantHeading(
        startLine,
        structure.headings,
      );
  ...
  return {
    ...
    stats: {
      ...
      avgChunkSize: Math.round(
        enrichedChunks.reduce((sum, c) => sum + c.content.length, 0) /
...
/**
 * Cleanup function để free memory (excluding embedding model for container reuse)
 */
function cleanup() {
  // Don't null out embeddingModel - keep it cached for Lambda container reuse
  // embeddingModel = null;
}
