...
async function getCachedFile(s3Key) {
  ...
  try {
    ...
    if (age < TMP_CACHE_TTL * 1000) {
      ...
      console.log(
        `Cache EXPIRED for ${s3Key} (age: ${Math.round(age / 1000)}s)`,
      );
...
exports.handler = async (event) => {
  ...
  console.log(
    `Cache stats - Metadata: ${metadataCache.size}, Chunks: ${chunkCache.size}`,
  );
...
async function processRecord(record) {
  try {
    ...
    console.log(
      `Indexed ${chunks.length} chunks for file: ${message.fileName}`,
    );
  } catch (error) {
